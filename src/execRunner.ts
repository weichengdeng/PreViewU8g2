...
function execFile(cmd: string, args: string[], opts: cp.SpawnOptions & { collect?: boolean } = {}): Promise<{ code: number, stdout: string, stderr: string }> {
...
export class ExecRunner {
  ...
  async runActive(editor: vscode.TextEditor, width: number, height: number): Promise<any | undefined> {
    const cfg = vscode.workspace.getConfiguration('u8g2Preview');
    const compiler = (cfg.get<string>('exec.compiler') || 'zig');
    const compilerPath = (cfg.get<string>('exec.compilerPath') || '').trim();
    const entrySymbol = (cfg.get<string>('exec.entrySymbol') || 'preview_entry');
    ...
    await fsp.writeFile(shim, SHIM_HPP, 'utf8');
    ...
    await fsp.writeFile(wrapper, wrapperSrc, 'utf8');
